...
BEGIN
  ...
  IF v_q IS NOT NULL THEN
    v_q_is_uuid := lower(v_q) ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$';
  END IF;
  ...
  filtered AS (
    ...
    WHERE
      ...
      AND (
        v_q IS NULL
        OR (
          (v_q_is_uuid AND b.id = lower(v_q)::uuid)
          ...
          OR COALESCE(b.public_id, '') ILIKE '%' || v_q || '%'
        )
      )
  ),
  counted AS (
