...
export async function resolveRoleFromUserAndClient(supabase, user) {
  ...
  try {
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) {
      console.warn("[business-gate] role lookup failed", {
        userId: user.id,
...
export async function findBusinessRowForOwner(supabase, ownerUserId) {
  ...
  try {
    const result = await supabase
      ...
      .maybeSingle();
    if (result.error) {
      console.warn("[business-gate] business row lookup failed", {
        ownerUserId,
...
export async function findBusinessProfileForOwner(supabase, ownerUserId) {
  ...
  try {
    const result = await supabase
      ...
      .maybeSingle();
    if (result.error) {
      console.warn("[business-gate] business profile lookup failed", {
        ownerUserId,
        message: result.error.message || String(result.error),
      });
    }
...
export async function requireBusinessRowOrOnboarding() {
  ...
  try {
    ...
    console.warn("[business-gate] auth lookup failed", {
      message: error?.message || String(error),
    });
    return;
  }
  ...
  try {
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) {
      console.warn("[business-gate] password state lookup failed", {
        userId: user.id,
        message: error.message || String(error),
      });
      return;
    }
    passwordRow = data ?? null;
  } catch (error) {
    console.warn("[business-gate] password state lookup failed", {
      userId: user.id,
      message: error?.message || String(error),
