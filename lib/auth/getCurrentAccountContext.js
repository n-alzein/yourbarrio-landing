...
export async function getCurrentAccountContext({
  ...
  const profileHasBlankEmail =
    Boolean(profile?.id) && !String(profile?.email || "").trim();
  const shouldRepairBlankEmail =
    profileHasBlankEmail && Boolean(authEmail) && !profileTombstoned;
  const shouldRecoverProfile =
    (!profile && !profileError) || profileTombstoned || shouldRepairBlankEmail;
    ...
    try {
      ...
      if (shouldLogProvisioningRecovery()) {
        console.info("[AUTH_PROFILE_BOOTSTRAP]", {
          ...
          profileTombstoned,
          profileBlankEmail: shouldRepairBlankEmail,
