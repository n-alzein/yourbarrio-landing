...
export async function requireRole(role) {
  ...
  const profileRole = profile?.role || user?.app_metadata?.role || null;
  let resolvedRole = supportModeActive
  ...
  try {
    const { data, error } = await supabase
      ...
      .eq("user_id", user.id);
    if (error) {
      console.warn("[AUTH_GUARD] admin_role_members lookup failed", {
        userId: user.id,
        message: error.message || String(error),
      });
    } else {
      adminRoleRows = Array.isArray(data) ? data : [];
    }
  } catch (error) {
    console.warn("[AUTH_GUARD] admin_role_members lookup failed", {
      userId: user.id,
  ...
  if (!supportModeActive && role === "customer" && !resolvedRole && !isAdmin) {
    // A live auth session can reach this guard before public.users has been
    // provisioned or repaired. Treat that as an authenticated customer shell;
    // profile recovery must not downgrade the first post-OAuth render.
  ...
  if (guardDiagEnabled) {
    console.warn("[AUTH_GUARD_DIAG] requireRole:resolved", {
      ...
      resolvedRole,
      profileRole,
      defaultedCustomerRole: !supportModeActive && role === "customer" && !profileRole && !isAdmin,
