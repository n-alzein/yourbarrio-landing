...
export function resolvePostAuthDestination({
  ...
  if (normalizedRole !== "business") {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.info("[auth-next] post-auth destination:", {
        ...
        safeNext: safeNext || null,
        destination,
        reason: "non_business_safe_next_or_role_landing",
      });
    }
    return destination;
  }
  ...
  if (process.env.NODE_ENV !== "production") {
    console.info("[auth-next] post-auth destination:", {
      role: normalizedRole || null,
      safeNext: normalizedNext || null,
      destination,
      reason: "business_password_or_onboarding_gate",
    });
  }
  return destination;
}
