...
export function isAdminProfile(profile) {
  ...
  return profile.role === "admin" || profile.is_internal === true;
}

export function resolvePostLoginTarget({ profile, role, next }) {
  const safeNext = getSafeRedirectPath(next);
  if (safeNext) return safeNext;

  if (isAdminProfile(profile) || role === "admin" || role === "internal") {
    return "/admin";
  }
