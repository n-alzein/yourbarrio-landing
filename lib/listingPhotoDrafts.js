...
export function revokeLocalPhotoDraftUrls(drafts) {
  ...
  drafts.forEach((draft) => {
    if (draft?.original?.previewUrl?.startsWith?.("blob:")) {
...
export function hydratePhotoDrafts(photoField, variantsField) {
  return extractStoredPhotoVariants(photoField, variantsField).map((variant) => ({
    ...
    tempUpload: null,
    mediaAssetId: variant.media_asset_id || variant.id || null,
    variants: variant.variants || null,
    normalization: null,
    enhancement: {
      background: variant.enhanced?.background || "white",
      isProcessing: false,
...
export function getDraftDisplayUrl(draft, variant = draft?.selectedVariant) {
  ...
  return draft.original?.publicUrl || draft.original?.previewUrl || draft.enhanced?.publicUrl || null;
}

export function getDraftFallbackDisplayUrl(draft, variant = draft?.selectedVariant) {
  if (!draft) return null;
  if (variant === "enhanced" && draft.enhanced?.publicUrl) {
    return draft.original?.publicUrl || draft.original?.previewUrl || null;
  }
  const primary = getDraftDisplayUrl(draft, variant);
  const fallback = draft.original?.previewUrl || null;
  return fallback && fallback !== primary ? fallback : null;
}

export function hasPendingPhotoUploads(drafts) {
  return (drafts || []).some((draft) =>
    ["local_preview", "uploading"].includes(draft?.uploadStatus)
  );
}

export function hasFailedPhotoUploads(drafts) {
  return (drafts || []).some((draft) => draft?.uploadStatus === "failed");
}
...
export function buildListingPhotoPayloadFromDrafts(drafts) {
  ...
  const variants = serializeStoredPhotoVariants(
    orderedDrafts.map((draft) => ({
      id: draft?.id || null,
      media_asset_id: draft?.mediaAssetId || draft?.tempUpload?.assetId || null,
      original: {
