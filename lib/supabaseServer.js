...
export const getSupabaseServerClient = cache(async () => {
  ...
  if (perfTimingEnabled()) {
    logServerTiming("getSupabaseServerClient", { buildMs });
  }
  return client;
});
...
export const getUserCached = cache(async (supabaseOverride) => {
  ...
  if (perfTimingEnabled()) {
    logServerTiming("getUserCached", { clientMs, fetchMs, totalMs: Math.round(clientMs + fetchMs) });
  }
...
export const getProfileCached = cache(async (userId, supabaseOverride) => {
  ...
  if (perfTimingEnabled()) {
    logServerTiming("getProfileCached", {
      clientMs,
      queryMs,
