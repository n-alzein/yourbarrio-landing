...
export function sanitizePublicProfile(profile) {
  ...
  return {
    ...
    avatar_media_asset_id: asTrimmedString(profile.avatar_media_asset_id),
    business_avatar_media_asset:
      profile.business_avatar_media_asset && typeof profile.business_avatar_media_asset === "object"
        ? { ...profile.business_avatar_media_asset }
        : null,
    cover_photo_url: asTrimmedString(profile.cover_photo_url),
    cover_media_asset_id: asTrimmedString(profile.cover_media_asset_id),
    business_cover_media_asset:
      profile.business_cover_media_asset && typeof profile.business_cover_media_asset === "object"
        ? { ...profile.business_cover_media_asset }
        : null,
    ...
    latitude: asNullableNumber(profile.latitude ?? profile.lat ?? profile.location_lat),
    longitude: asNullableNumber(profile.longitude ?? profile.lng ?? profile.location_lng),
...
export function sanitizeGalleryPhotos(items) {
  return asList(items)
    ...
    .map((item, index) => ({
      ...
      media_asset_id: asTrimmedString(item.media_asset_id),
      media_asset:
        item.media_asset && typeof item.media_asset === "object"
          ? { ...item.media_asset }
          : null,
    }));
}
...
export function sanitizeReviews(items) {
  return asList(items)
    ...
    .map((item, index) => ({
      ...
      author_display_name: asTrimmedString(item.author_display_name),
      author_profile:
        item.author_profile && typeof item.author_profile === "object"
          ? {
              ...item.author_profile,
              user_id: asTrimmedString(item.author_profile.user_id),
              display_name: asTrimmedString(item.author_profile.display_name),
              avatar_url: asTrimmedString(item.author_profile.avatar_url),
            }
          : null,
      created_at: item.created_at || null,
