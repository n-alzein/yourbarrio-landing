...
export function resolveImageSrc(input, fallback = "/business-placeholder.png") {
  const raw =
    typeof input === "string"
      ? input.trim()
      : input != null
  ...
  if (failedSrcs.has(raw)) return fallbackSrc;
  ...
  if (
    ...
    if (supabaseUrl) {
      if (failedSrcs.has(supabaseUrl)) return fallbackSrc;
      return supabaseUrl;
    }
  }
  ...
  if (relativeCandidate && relativeCandidate.startsWith("/")) {
    if (failedSrcs.has(relativeCandidate)) return fallbackSrc;
