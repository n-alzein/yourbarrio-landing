...
export async function getHomeListings({ limit = 80, location, category } = {}) {
  ...
  let query = supabase
    ...
    .select(
      "id,title,price,category,category_id,city,photo_url,photo_variants,business_id,created_at,inventory_status,inventory_quantity,low_stock_threshold,inventory_last_updated_at"
    )
  ...
  if (categoryNames.length || categorySlugs.length) {
    ...
    return { data: Array.from(deduped.values()).slice(0, limit), error: null };
  }
