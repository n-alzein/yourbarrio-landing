...
export default function BusinessesPage() {
  ...
  async function loadBusinesses() {
    ...
    // Merge static examples + real businesses
    const realBusinesses = (data || []).map((biz) => ({
      name: biz.business_name || "Untitled Business",
      category: biz.category || "General",
      rating: 4.7, // You can later replace with real rating
      description: biz.description || "No description provided.",
      profile_photo_url: biz.profile_photo_url,
    }));

    setBusinesses([...staticBusinesses, ...realBusinesses]);
    setLoading(false);
  }
  ...
  return (
    <div className="min-h-screen text-white relative pt-32 px-6 max-w-7xl mx-auto">
      ...
      <div className="grid gap-8 md:grid-cols-3">
        ...
        {loading && <p className="text-white/70">Loading businesses…</p>}
